...
const PREDICT_URL: string = 'http://max-image-caption-generator.codait-prod-41208c73af8fca213512856c7a09db52-0000.us-east.containers.appdomain.cloud/model/predict';
...
export class MediaService {
  ...
  async create(files: any, user: any) {
    ...
    for (let i = 0; i < files.length; i++) {
      const file: any = files[i],
        info: any = this.config.get('STORAGE_TYPE').toLowerCase() === 'DISC' 
          ? await probe(createReadStream(file.path))
          : await probe(`${this.config.get('GOOGLE_URL')}/${file.filename}`),
        url: string = `https://api.imagga.com/v2/tags?image_url=${encodeURIComponent(`${this.config.get('GOOGLE_URL')}/${file.filename}`)}`;


      const { result }: any = await request(url, 'GET', {
        headers: {
          'Authorization': 'Basic YWNjXzYzMTE1ZDUxOWMxODMxMzo2YjFkZjgyZDI2ODFjNzhmZjJkNmVhZjA5OTI0ZGE1Zg=='
        }
      })

      const tags: Array<string> = result.tags.filter((t: any) => t.confidence > 20).map((t: any) => {
        return t.tag.en;
      })
  ...
  async copy(files: Array<any>, user: any) {
    ...
    const tostorage: Array<Promise<any>> = [],
      media: Array<any> = [],
      bucket: Storage.Bucket = this.storage.bucket(this.config.get('GOOGLE_BUCKET'));

    files.forEach((file: any) => {
      tostorage.push( new Promise(async (resolve: Function, reject: Function) => {
        const ext: string = extname(file.url),
          id: string = uuid(),
          _file = bucket.file(`${user.workspace.id}/${id}${ext}`),
          info = await probe(file.url)
        get(file.url).pipe(_file.createWriteStream({
          public: true,
          ...
          media.push({
            tags: file.caption.split(',').map((tag: string) => tag.trim()),
            width: info.width,
            height: info.height,
            name: `${id}${ext}`,
            type: info.mime,
  ...
  async delete(id: string, user: any): Promise<any> {
    ...
    if(this.config.get('STORAGE_TYPE').toLowerCase() === 'google') {
      ...
      unlink(join(this.config.get('UPLOADS_PATH'), media.name), () => {});
    }
